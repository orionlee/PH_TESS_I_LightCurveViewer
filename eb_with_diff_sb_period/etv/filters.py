...
def filt1d(array, nmed, nlin, fill=False, circ=False, kernel="box"):
    ...
    # Filter
    if nmed > 1:
        work = signal.medfilt(work2, nmed)
    else:
        work = work2
    if kernel == "gauss":
...
def NIF(
    array, nmed, nlin, nsig=3.0, prefilt=False, fill=True, circ=False, verbose=False
):
    """
