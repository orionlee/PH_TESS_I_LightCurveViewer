...
def download_lightcurvefiles(target, mission=('Kepler', 'K2', 'TESS'), download_dir=None, use_cache='yes'):
    '''
    Wraps `lightkurve.serach.search_lightcurvefile()` and the subsequent `lightkurve.serach.SearchResult.download_all()` calls,
    with the option of caching, so that for a given search, if the the result has been downloaded, the cache will be used.
    
    The parameters all propagate to the underlying `search_lightcurvefile()` and `download_all()` calls. The lone exception
    is `use_cache`.
    
    Parameters
    ----------
    use_cache : str, must be one of 'yes', 'no'
        'yes': cache will be used when available. 'no': cache will not be used.\n
        OPEN: support an  option of 'fallback': cache will be used when offline.\n
        OPEN: for now, actual download lightcurve cache will still be used if available irrespective of the settings.
    ...
    if 'no' == use_cache:
        ...
        if result_file_ids is not None:
            ...
            return lk.collections.LightCurveFileCollection(list(map(lambda f: lk.open(f), result_files)))
        else:
            return _search_and_cache(target, mission, download_dir)
    else:
...
def _to_product_identifiers(search_res):
    ...
    '''
    return list(map(lambda e: e['obs_collection'] +  '/' + e['obs_id'] + '/' + e['productFilename']
         , search_res.table))
...
def _load_search_result_product_identifiers(download_dir, key):    
    ...
    try:
        ...
        if err.errno != 2:
            log.warning('Unexpected OSError in retrieving cached search result: ' + err)
        return None
